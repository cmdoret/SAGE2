...
cropannot <- list('metabolic'='met.','process'='proc','cellular'='cell',
                  'extracellular'='extracell.','intracellular'='intracell',
                  'biosynthetic'='biosynt.','internal'='int','external'='ext',
                  'activity'='act.','structure'='struct.','enzyme'='enz.',
                  'coenzyme'='coenz.','Phosphorus'='Phos.','transporter'='transp.')

#short_annot <- function(long){}
...
GOcbar <- function(GO_folder){
  ...
  for(f in files[2:length(files)]){  # Iterating over all files (besides the first one)
    ...
    bp <- bp + theme_minimal() + theme(axis.text.x = element_blank()) + 
      geom_hline(aes_string(yintercept=0)) + facet_grid(~term_type, scales='free', drop=T, 
                                                        labeller = as_labeller(clean_labels))
